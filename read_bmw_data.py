...
def clean_and_preprocess_data(df):
    ...
    for col in numerical_cols:
        ...
        if outliers_count > 0:
            print(
                f"   - {col}: Removed {outliers_count} outliers (range: {lower_bound:.2f} - {upper_bound:.2f})"
            )
    ...
    for col in text_columns:
        if col in cleaned_df.columns:
            ...
            if unique_before != unique_after:
                print(
                    f"   - {col}: Standardized casing ({unique_before} -> {unique_after} unique values)"
                )
    ...
    invalid_years = cleaned_df[
        (cleaned_df["Year"] < 2020) | (cleaned_df["Year"] > 2024)
    ]
    ...
    invalid_engine = cleaned_df[
        (cleaned_df["Engine_Size_L"] < 1.5) | (cleaned_df["Engine_Size_L"] > 5.0)
    ]
    ...
    cleaned_df["Price_Category"] = pd.cut(
        cleaned_df["Price_USD"], bins=price_bins, labels=price_labels
    )
    ...
    cleaned_df["Mileage_Category"] = pd.cut(
        cleaned_df["Mileage_KM"], bins=mileage_bins, labels=mileage_labels
    )
    print("   - Added 'Mileage_Category' column")

    # 8. Sort data for consistency
    cleaned_df = cleaned_df.sort_values(["Year", "Model", "Region"]).reset_index(
        drop=True
    )
    ...
    print(
        f"Rows removed: {initial_rows - len(cleaned_df)} ({((initial_rows - len(cleaned_df))/initial_rows*100):.2f}%)"
    )
