...
def main():
    ...
    try:
        ...
        logger.info(f"Analysis complete - {analysis_data['overview']['total_records']} records analyzed")
    ...
    try:
        ...
        evaluation = evaluator.evaluate_report(analysis_data, llm_insights, html_path, md_path)
        
        # Display evaluation summary
        print(f"✓ Report evaluation complete!")
        print(f"  - Overall Score: {evaluation['overall_score']:.1f}/100")
        print(f"  - Grade: {evaluation['grade']}")
        print(f"  - Correctness: {evaluation['correctness']['score']:.1f}/100 [{evaluation['correctness']['status']}]")
        print(f"  - Completeness: {evaluation['completeness']['score']:.1f}/100 [{evaluation['completeness']['status']}]")
        print(f"  - Readability: {evaluation['readability']['score']:.1f}/100 [{evaluation['readability']['status']}]")
        
        # Generate and save evaluation report
        eval_report = evaluator.generate_evaluation_report(evaluation)
        eval_report_path = os.path.join(output_dir, "evaluations", f"evaluation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        ...
        logger.warning(f"Error during evaluation (non-critical): {str(e)}", exc_info=True)
        print(f"⚠ Warning: Report evaluation encountered issues but analysis completed successfully")
        print()
