...
class ReportEvaluator:
    ...
    def evaluate_report(self, 
        ...
        logger.info(f"Evaluation complete. Overall score: {evaluation['overall_score']:.2f}/100 ({evaluation['grade']})")
        return evaluation
    
    def _evaluate_correctness(self, analysis_data: Dict[str, Any], llm_insights: Dict[str, str]) -> Dict[str, Any]:
        """
        ...
        required_sections = ['overview', 'yearly_trends', 'regional_performance', 
                           'model_performance', 'price_analysis']
        ...
        for key, insight in llm_insights.items():
            if not insight or len(insight) < 50:
                score -= 5
                issues.append(f"Insufficient insight for {key}: only {len(insight)} characters")
    ...
    def _evaluate_completeness(self, analysis_data: Dict[str, Any], llm_insights: Dict[str, str]) -> Dict[str, Any]:
        """
        ...
        expected_insights = [
            'executive_summary', 'yearly_analysis', 'regional_analysis',
            'model_analysis', 'drivers_analysis', 'creative_insights', 'recommendations'
        ]
    ...
    def _evaluate_data_quality(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'regional_performance' in analysis_data:
            ...
            sales_values = [v.get('total_sales', 0) for v in regional.values() if isinstance(v, dict)]
            if sales_values:
    ...
    def generate_evaluation_report(self, evaluation: Dict[str, Any]) -> str:
        ...
        if evaluation['overall_score'] >= 90:
            ...
            report.append("✗ Report quality below acceptable standards. Significant improvements required.")
