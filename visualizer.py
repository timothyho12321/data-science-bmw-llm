...
class BMWDataVisualizer:
    ...
    def plot_regional_performance(self):
        ...
        # Sales by region
        regional_sales = (
            self.df.groupby("Region")["Sales_Volume"].sum().sort_values(ascending=True)
        )
        ax1.barh(
            regional_sales.index, regional_sales.values, color="#ff7f0e", alpha=0.7
        )
    ...
    def plot_model_performance(self):
        ...
        # Sales by model
        model_sales = (
            self.df.groupby("Model")["Sales_Volume"].sum().sort_values(ascending=True)
        )
        bars = ax.barh(
            model_sales.index, model_sales.values, color="#d62728", alpha=0.7
        )
        ...
        ax.set_title(
            "Sales Volume by Model (Top 3 in Green)", fontsize=14, fontweight="bold"
        )
    ...
    def plot_price_distribution(self):
        ...
        ax1.hist(
            self.df["Price_USD"], bins=30, color="#9467bd", alpha=0.7, edgecolor="black"
        )
        ...
        price_sales_agg = self.df.groupby(price_bins).agg(
            {"Price_USD": "mean", "Sales_Volume": "sum"}
        )

        ax2.scatter(
            price_sales_agg["Price_USD"],
            price_sales_agg["Sales_Volume"],
            alpha=0.6,
            s=100,
            color="#8c564b",
        )
    ...
    def plot_fuel_type_trends(self):
        ...
        ax1.set_title(
            "Fuel Type Sales Trends Over Time", fontsize=14, fontweight="bold"
        )
        ax1.set_xlabel("Year", fontsize=12)
    ...
    def plot_revenue_analysis(self):
        ...
        # Revenue by year
        yearly_revenue = (
            self.df.groupby("Year")["Total_Revenue"].sum() / 1e9
        )  # Convert to billions
        ...
        model_revenue = (
            self.df.groupby("Model")["Total_Revenue"]
            .sum()
        ...
        ax2.barh(
            range(len(model_revenue)), model_revenue.values, color="#2ca02c", alpha=0.7
        )
        ...
        model_metrics = self.df.groupby("Model").agg(
            {"Sales_Volume": "sum", "Total_Revenue": "sum"}
        )
        ax4.scatter(
            model_metrics["Sales_Volume"],
            model_metrics["Total_Revenue"] / 1e6,
            s=100,
            alpha=0.6,
            color="#d62728",
        )
        ax4.set_title(
            "Sales Volume vs Revenue by Model", fontsize=14, fontweight="bold"
        )
        ...
        for model, row in model_metrics.iterrows():
            ax4.annotate(
                model,
                (row["Sales_Volume"], row["Total_Revenue"] / 1e6),
                fontsize=8,
                alpha=0.7,
            )
    ...
    def plot_correlation_heatmap(self):
        ...
        ax.set_title(
            "Correlation Heatmap of Key Metrics", fontsize=14, fontweight="bold"
        )
    ...
    def plot_model_category_performance(self):
        ...
        ax1.barh(
            category_sales.index, category_sales.values, color="#e377c2", alpha=0.7
        )
        ...
        category_yearly = (
            self.df.groupby(["Year", "Model_Category"])["Sales_Volume"]
            .sum()
            .unstack(fill_value=0)
        )
