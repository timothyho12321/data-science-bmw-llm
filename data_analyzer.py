...
class BMWDataAnalyzer:
    ...
    def get_overview(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_revenue': float(self.df['Total_Revenue'].sum()) if 'Total_Revenue' in self.df.columns else None,
    ...
    def analyze_yearly_trends(self) -> Dict[str, Any]:
        ...
        yearly_stats = self.df.groupby('Year').agg({
            ...
            'Price_USD': 'mean',
            'Total_Revenue': 'sum' if 'Total_Revenue' in self.df.columns else 'count'
        }).round(2)
        ...
        return {
            ...
            'total_growth': float(((yearly_sales.iloc[-1] - yearly_sales.iloc[0]) / yearly_sales.iloc[0] * 100))
        }
    ...
    def analyze_regional_performance(self) -> Dict[str, Any]:
        ...
        regional_sales = self.df.groupby('Region')['Sales_Volume'].sum().sort_values(ascending=False)
        
        return {
            'regional_sales': regional_sales.to_dict(),
            'top_region': regional_sales.idxmax(),
            'bottom_region': regional_sales.idxmin(),
            'regional_market_share': (regional_sales / regional_sales.sum() * 100).round(2).to_dict(),
            'avg_price_by_region': self.df.groupby('Region')['Price_USD'].mean().sort_values(ascending=False).to_dict()
        }
    
    def analyze_model_performance(self) -> Dict[str, Any]:
        """Analyze performance by model"""
        model_sales = self.df.groupby('Model')['Sales_Volume'].sum().sort_values(ascending=False)
        model_revenue = self.df.groupby('Model')['Total_Revenue'].sum().sort_values(ascending=False) if 'Total_Revenue' in self.df.columns else None
        
        # Model category analysis if available
        if 'Model_Category' in self.df.columns:
            category_sales = self.df.groupby('Model_Category')['Sales_Volume'].sum().sort_values(ascending=False)
        ...
        return {
            ...
            'model_market_share': (model_sales / model_sales.sum() * 100).round(2).to_dict(),
            'avg_price_by_model': self.df.groupby('Model')['Price_USD'].mean().sort_values(ascending=False).to_dict(),
    ...
    def analyze_price_drivers(self) -> Dict[str, Any]:
        ...
        if 'Price_Category' in self.df.columns:
            price_cat_sales = self.df.groupby('Price_Category')['Sales_Volume'].sum().to_dict()
        else:
        ...
        self.df['Price_Bin'] = pd.cut(self.df['Price_USD'], bins=price_bins, labels=price_labels)
        
        price_segment_sales = self.df.groupby('Price_Bin')['Sales_Volume'].sum().to_dict()
        
        return {
            'price_category_sales': price_cat_sales,
            'price_segment_sales': price_segment_sales,
            'avg_sales_by_price_segment': self.df.groupby('Price_Bin')['Sales_Volume'].mean().to_dict(),
            'price_elasticity_indicator': float(self.df['Price_USD'].corr(self.df['Sales_Volume']))
        }
    
    def analyze_fuel_type_trends(self) -> Dict[str, Any]:
        """Analyze fuel type preferences over time"""
        fuel_sales = self.df.groupby('Fuel_Type')['Sales_Volume'].sum().sort_values(ascending=False)
        ...
        return {
            ...
            'fuel_market_share': (fuel_sales / fuel_sales.sum() * 100).round(2).to_dict(),
    ...
    def analyze_transmission_preference(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'transmission_market_share': (trans_sales / trans_sales.sum() * 100).round(2).to_dict(),
    ...
    def analyze_revenue(self) -> Dict[str, Any]:
        ...
        revenue_by_model = self.df.groupby('Model')['Total_Revenue'].sum().sort_values(ascending=False)
        revenue_by_region = self.df.groupby('Region')['Total_Revenue'].sum().sort_values(ascending=False)
